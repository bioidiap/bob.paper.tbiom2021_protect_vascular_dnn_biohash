...
sub_directory = 'img-AE-BHsh-100/baseline'  # pre-processed and extracted features will be placed here, along with the enrolled models
...
#             nn.Dropout(0.2),
            nn.Flatten(),
            nn.Linear(8*21*256,embedding_layer_length),
            #nn.BatchNorm2d(128),
            nn.ReLU(),
            #nn.Dropout(0.2)
        )
        ...
        self.fc = nn.Sequential( 
            nn.Linear(embedding_layer_length,8*21*256),
    ...
    def forward(self, anchor, positive, negative):
        ...
        input_to_decoder_anchor   = self.fc(embedded_anchor).view(-1,256,8,21)
        input_to_decoder_positive = self.fc(embedded_anchor).view(-1,256,8,21)
        input_to_decoder_negative = self.fc(embedded_anchor).view(-1,256,8,21)
...
class ImageEncoder (Extractor):
    ...
    def __call__(self, image):
        ...
        finger_image_ = np.zeros([1,1,finger_image.shape[0]-4,finger_image.shape[1]-3])
        finger_image_[0,0,:,:] = finger_image[2:-2,2:-1]/255.0
