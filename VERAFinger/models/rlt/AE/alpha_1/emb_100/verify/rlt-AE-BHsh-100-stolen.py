...
#             nn.Dropout(0.2),
            nn.Flatten(),
            nn.Linear(5*13*256,embedding_layer_length),
            #nn.BatchNorm2d(128),
            nn.ReLU(),
            #nn.Dropout(0.2)
        )
        ...
        self.fc = nn.Sequential( 
            nn.Linear(embedding_layer_length,5*13*256),
    ...
    def forward(self, anchor, positive, negative):
        ...
        input_to_decoder_anchor   = self.fc(embedded_anchor).view(-1,256,5,13)
        input_to_decoder_positive = self.fc(embedded_anchor).view(-1,256,5,13)
        input_to_decoder_negative = self.fc(embedded_anchor).view(-1,256,5,13)
...
class RepeatedLineTracking (Extractor):
    ...
    def __call__(self, image):
        ...
        img_veins_bin=np.zeros([1,1,rlt.shape[0]-2,rlt.shape[1]+5])
        img_veins_bin[0,0,:,2:-3] = rlt[1:-1,:]#/255.
