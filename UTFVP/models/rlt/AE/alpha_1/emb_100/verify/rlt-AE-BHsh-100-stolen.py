...
class Net(nn.Module):
    ...
    def forward(self, anchor, positive, negative):
        ...
        input_to_decoder_anchor   = self.fc(embedded_anchor).view(-1,256,7,13)
        input_to_decoder_positive = self.fc(embedded_anchor).view(-1,256,7,13)
        input_to_decoder_negative = self.fc(embedded_anchor).view(-1,256,7,13)
...
class RepeatedLineTracking (Extractor):
    ...
    def __call__(self, image):
        ...
        img_veins_bin=np.zeros([1,1,rlt.shape[0]-10,rlt.shape[1]+7])
        img_veins_bin[0,0,:,3:-4] = rlt[5:-5,:]#/255.
