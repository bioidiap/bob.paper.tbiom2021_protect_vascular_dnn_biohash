...
all_feats = np.load('../../../../../data/all_rlt_aug_feats.npy')
...
all_input = np.zeros([all_feats.shape[0], all_feats.shape[1], all_feats.shape[2]-10, all_feats.shape[3]+7])
all_input[:,:,:,3:-4] = all_feats[:,:,5:-5,:]#/255.
...
#             nn.Dropout(0.2),
            nn.Flatten(),
            nn.Linear(7*13*256,embedding_layer_length),
            #nn.BatchNorm2d(128),
            nn.ReLU(),
            #nn.Dropout(0.2)
        )
        ...
        self.fc = nn.Sequential( 
            nn.Linear(embedding_layer_length,7*13*256),
