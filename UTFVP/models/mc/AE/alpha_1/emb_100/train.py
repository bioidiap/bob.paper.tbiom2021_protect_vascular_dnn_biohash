...
#(380, 672) -> (384, 672)
all_input = np.zeros([all_feats.shape[0], all_feats.shape[1], all_feats.shape[2]-6, all_feats.shape[3]-10])
all_input[:,:,:,:] = all_feats[:,:,3:-3,5:-5]#/255.
...
#             nn.Dropout(0.2),
            nn.Flatten(),
            nn.Linear(12*21*256,embedding_layer_length),
            #nn.BatchNorm2d(128),
            nn.ReLU(),
            #nn.Dropout(0.2)
        )
        ...
        self.fc = nn.Sequential( 
            nn.Linear(embedding_layer_length,12*21*256),
