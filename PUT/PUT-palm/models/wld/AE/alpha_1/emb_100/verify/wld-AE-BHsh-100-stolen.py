...
sub_directory = 'wld-AE-BHsh-100-stolen/baseline'  # pre-processed and extracted features will be placed here, along with the enrolled models
...
#             nn.Dropout(0.2),
            nn.Flatten(),
            nn.Linear(3*4*256,embedding_layer_length),
            #nn.BatchNorm2d(128),
            nn.ReLU(),
            #nn.Dropout(0.2)
        )
        ...
        self.fc = nn.Sequential( 
            nn.Linear(embedding_layer_length,3*4*256),
    ...
    def forward(self, anchor, positive, negative):
        ...
        input_to_decoder_anchor   = self.fc(embedded_anchor).view(-1,256,3,4)
        input_to_decoder_positive = self.fc(embedded_anchor).view(-1,256,3,4)
        input_to_decoder_negative = self.fc(embedded_anchor).view(-1,256,3,4)
...
class WideLineDetector (Extractor):
    ...
    def wide_line_detector(self, finger_image, mask):
        ...
        img_veins_bin_=np.zeros([1,1,img_veins_bin.shape[0]+1,img_veins_bin.shape[1]+3])
        img_veins_bin_[0,0,:-1,1:-2]=img_veins_bin[:,:]#/255.
