...
all_feats = np.load('../../../../../data/wrist_all_rlt_aug_feats.npy')
...
import pickle
with open('../../../../../data/wrist_all_client_ids_aug.pkl','rb') as f:
    all_client_ids=pickle.load(f)

all_input = np.zeros([all_feats.shape[0], all_feats.shape[1], all_feats.shape[2]-12, all_feats.shape[3]+7])
all_input[:,:,:,3:-4] = all_feats[:,:,6:-6,:]#/255.
...
#             nn.Dropout(0.2),
            nn.Flatten(),
            nn.Linear(7*10*256,embedding_layer_length),
            #nn.BatchNorm2d(128),
            nn.ReLU(),
            #nn.Dropout(0.2)
        )
        ...
        self.fc = nn.Sequential( 
            nn.Linear(embedding_layer_length,7*10*256),
    ...
    def forward(self, anchor, positive, negative):
        ...
        input_to_decoder_anchor   = self.fc(embedded_anchor).view(-1,256,7,10)
        input_to_decoder_positive = self.fc(embedded_anchor).view(-1,256,7,10)
        input_to_decoder_negative = self.fc(embedded_anchor).view(-1,256,7,10)
